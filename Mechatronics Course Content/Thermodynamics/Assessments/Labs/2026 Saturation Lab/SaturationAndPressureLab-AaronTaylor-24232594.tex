...
laboratory calibration. Experimental temperatures were compared with linearly
interpolated values from the saturated-water pressure table.
...
$T=30.899P^{0.2563}$ for the steam-table data. Experimental temperatures were
...
\begin{equation}
  ...
  \label{eq:power}
\end{equation}
where $A$ and $n$ were determined separately for the experimental and
steam-table datasets.
...
\begin{enumerate}
  ...
  \item Gauge pressures were converted to absolute pressure, resistances to
        temperature, and theoretical saturation temperatures were interpolated
        from Table A--5.
  \item Power-law fits were obtained by linear regression of $\ln T$ against
        $\ln P$, and temperature errors were calculated for all nine points.
\end{enumerate}
...
\begin{table}[H]
  \centering
  \caption{Experimental readings, processed temperatures and comparison with
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.91\textwidth]{Figure 2 - Temperature versus absolute pressure.png}
  \caption{Experimental PT100-derived temperature and interpolated steam-table
  saturation temperature plotted against absolute pressure, with power-law
  trendlines.}
...
\begin{figure}[H]
  ...
  \caption{Signed experimental temperature error relative to the interpolated
  saturated-water values.}
  \label{fig:error}
\end{figure}
...
The experimental exponent, $n=0.2146$, was lower than the theoretical value of
$0.2563$. Consequently, the experimental equation predicts too little
